...
model1 <- lm(data = data, depression ~ age + gender + stress + loneliness + selfesteem)
...
- We would tell the whole story

- If we focus only on the final model, this would be the same value as only running the final model

- The research question is answered by showing the relationship between the models

## Generating a table of the models
...
## Example table of models

![](images/regression_table.png){width="100%"}
